...
- Canonical JSON v1 is now centralized in a private helper and documented with
  ...
  TasteCard, state, ContextPack, conflict-resolution, hygiene-report,
...
- `AccessDecision` now carries machine-readable access metadata
  ...
  now use stable record-type names, `allowed_record_types` preserves legacy
  candidate/ledger discriminator aliases such as `claim` and `fact`, MCP
