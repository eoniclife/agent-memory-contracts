...
- **`bundle_fingerprint(records)`** for content-addressed bundle digests.
  ...
  by default, with opt-in duplicate modes (`last`, `first`, `raise`).
...
# Strict imports can fail on repeated semantic ids instead of silently
# applying the legacy last-write-wins policy.
